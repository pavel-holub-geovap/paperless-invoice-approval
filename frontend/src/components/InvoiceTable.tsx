      ...
      <table>
        ...
        <tbody>{rows.map((row) => (
          <tr key={row.id} onClick={() => onOpen(row.id)} tabIndex={0} onKeyDown={(e) => e.key === "Enter" && onOpen(row.id)}>
            <td><strong>{row.invoice_number || row.title || `Paperless #${row.paperless_document_id}`}</strong><small>Paperless #{row.paperless_document_id} · revize {row.current_revision_number}{row.uploaded_by ? ` · nahrál ${row.uploaded_by}` : ""}</small>{row.upload_origin==="APPROVER"&&<small className="source-badge">Nahrál schvalovatel · {row.queue_manager_reviewed?"revize zkontrolována":"čeká na kontrolu"}</small>}</td>
