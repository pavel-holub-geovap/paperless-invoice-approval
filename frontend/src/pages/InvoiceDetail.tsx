    ...
    <div className="detail-grid">
      ...
      <div className="work-panel">
        ...
        {isManager&&<div className="card pohoda-export"><div className="card-title"><div><h2>POHODA export</h2><p>Deterministické XML pro ruční import; stažení samo nepotvrzuje import.</p></div>{invoice.pohoda_export&&<StatusBadge value={invoice.pohoda_export.status}/>}</div>
          ...
          <div className="decision-buttons">{invoice.status==="APPROVED"&&invoice.pohoda_export?.status!=="XSD_VALID"&&<button className="button primary" disabled={!actionable||Boolean(pending)||pohodaConfig?.identification==="NOT_CONFIGURED"} onClick={()=>generateXml()}>{pending==="xml"?"Generuji a validuji…":"Vygenerovat XML"}</button>}{invoice.pohoda_export?.status==="XSD_VALID"&&<><a className="button secondary" href={`/api/exports/artifacts/${invoice.pohoda_export.id}/xml`}>Stáhnout XML</a>{actionable&&<><a className="button secondary" href={`/api/invoices/${invoice.id}/pdf`}>Stáhnout PDF</a><button className="button secondary" disabled={Boolean(pending)} onClick={()=>void createZip()}>{pending==="zip"?"Vytvářím ZIP…":"Stáhnout ZIP"}</button><button className="button warning" disabled={Boolean(pending)} onClick={()=>{const reason=window.prompt("Důvod re-exportu (volitelný)");if(reason!==null)generateXml(reason)}}>{pending==="xml"?"Generuji…":"Re-export"}</button></>}{!actionable&&<span className="muted">Nový export, PDF a ZIP jsou zablokované.</span>}</>}{invoice.status==="EXPORT_CREATED"&&invoice.pohoda_export&&<button className="button primary" disabled={!actionable||Boolean(pending)} onClick={()=>void markImported()}>{pending==="import"?"Potvrzuji…":"OZNAČIT JAKO IMPORTOVÁNO DO POHODY"}</button>}</div>
