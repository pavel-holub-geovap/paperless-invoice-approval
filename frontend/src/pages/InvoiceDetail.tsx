...
export function InvoiceDetail({ invoice, user, onBack, onRefresh }: { invoice: Invoice; user: User; onBack: () => void; onRefresh: () => void }) {
  ...
  const [allocationRows, setAllocationRows] = useState(() => invoice.allocations.map((a) => ({ cost_center_id: a.cost_center.id, amount: String(a.amount), percentage: String(a.percentage ?? ""), note: a.note ?? "" })));
  ...
  useEffect(() => { setForm(formFromInvoice(invoice)); setAllocationRows(invoice.allocations.map((a)=>({cost_center_id:a.cost_center.id,amount:String(a.amount),percentage:String(a.percentage??""),note:a.note??""}))); setApproverChoices(Object.fromEntries(invoice.allocations.map((a)=>[a.id,a.assignments.map((x)=>x.approver_subject)]))); }, [invoice]);
  useEffect(() => {
  ...
  const saveAllocations = () => void call(`/invoices/${invoice.id}/allocations`, { method: "PUT", body: JSON.stringify({ allocations: allocationRows.map((row) => ({ cost_center_id: row.cost_center_id, note: row.note || null, ...(allocationMode === "amount" ? { amount: row.amount } : { percentage: row.percentage }) })) }) });
