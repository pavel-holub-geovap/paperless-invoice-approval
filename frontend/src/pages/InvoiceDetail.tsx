...
const editableFields = [
  ...
  ["total_without_vat", "Základ bez DPH"], ["total_vat", "DPH celkem"], ["total_amount", "Celkem"], ["description", "Poznámka"],
] as const;
...
export function InvoiceDetail({ invoice, user, onBack, onRefresh }: { invoice: Invoice; user: User; onBack: () => void; onRefresh: (updated?: Invoice) => void }) {
  ...
  useEffect(() => {
    ...
    if (user.roles.includes("QUEUE_MANAGER")) {
      ...
      ]).then(([[centers, history], users, permissions, config]) => {
        setCentres(centers); setApprovers(users); setSectionPermissions(permissions); setAudit(history); setPohodaConfig(config);
      });
    } else {
      void common.then(([centers, history]) => { setCentres(centers); setAudit(history); });
    }
  ...
  const roundingRowIndex = Number(roundingValidation?.details?.row ?? 0) - 1;
    ...
    <div className="detail-grid">
      ...
      <div className="work-panel">
        <div className="card classification-card"><div className="card-title"><div><h2>Klasifikace a zpracování</h2><p>Typ dokladu a režim jsou nezávislé údaje.</p></div><StatusBadge value={invoice.classification.extraction_source}/></div>
          <div className="form-grid"><label>Typ dokladu<select disabled={!canPrepare||Boolean(pending)} value={documentType} onChange={(e)=>setDocumentType(e.target.value)}>{documentTypeOptions.map((value)=><option key={value} value={value}>{documentTypeLabel(value)}</option>)}</select></label><label>Režim<select disabled={!canPrepare||Boolean(pending)} value={processingMode} onChange={(e)=>setProcessingMode(e.target.value)}>{processingModeOptions.map((value)=><option key={value} value={value}>{processingModeLabel(value)}</option>)}</select></label></div>
        ...
        <div className="card vat-card"><div className="card-title"><div><h2>DPH a zaokrouhlení</h2><p>Hodnoty vytištěné na faktuře zůstávají autoritativní; přepočet je pouze kontrola.</p></div></div>
          ...
          {roundingLine&&roundingValidation&&<div className="alert warning"><strong>⚠ Rozdíl je pravděpodobně způsoben položkou Zaokrouhlení.</strong> Částka {shown(roundingValidation.actual)} Kč.</div>}
        ...
        <div className="card"><div className="card-title"><div><h2>Sekce dokladu</h2><p>Každá část patří sekci; schvalovatel může použít jen sekce, ke kterým má oprávnění.</p></div>{canPrepare&&<button className="button secondary" disabled={Boolean(pending)} onClick={()=>changeAllocationRows([...allocationRows,{cost_center_id:centres[0]?.id || "",amount:"0.00",percentage:"0",note:"",vat_breakdown:"[]"}])}>Přidat řádek</button>}</div>
        ...
        {!isManager&&invoice.paperless.uploaded_by_subject===user.subject&&<div className="card actions"><h2>Příprava a předání</h2>{invoice.queue_review?.submitted_at?<div className="alert info"><strong>Čeká na kontrolu správce fronty.</strong> Předáno {pragueDateTime(invoice.queue_review.submitted_at)}. Případné změny vytvoří správce fronty v nové revizi.</div>:<><p>Zkontrolujte originál, doplňte sekce a schvalte své sekce v záložce „Ke schválení“. Potom doklad předejte.</p><div><button className="button secondary" disabled={!actionable||Boolean(pending)} onClick={()=>void call("original", `/invoices/${invoice.id}/confirm-original`,{method:"POST"},"Kontrola originálu byla potvrzena.")}>{pending==="original"?"Potvrzuji…":"Potvrdit kontrolu originálu"}</button><a className="button secondary" href="/approvals">Schválit vlastní sekce</a><button className="button primary" disabled={!actionable||Boolean(pending)} onClick={()=>void call("queue-review", `/invoices/${invoice.id}/submit-for-review`,{method:"POST"},"Doklad byl předán správci fronty.")}>{pending==="queue-review"?"Předávám…":"Předat správci fronty"}</button></div></>}</div>}
