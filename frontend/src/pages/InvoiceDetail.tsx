...
import type { AuditEvent, CostCenter, Invoice, PohodaConfig, SectionPermission, User, UserReference } from "../types";
...
const editableFields = [
  ...
  ["total_without_vat", "Základ bez DPH"], ["total_vat", "DPH celkem"], ["total_amount", "Celkem"], ["rounding_amount", "Zaokrouhlení"], ["description", "Poznámka"],
] as const;
...
const draftSnapshot = (invoice: Invoice) => JSON.stringify({
  ...
  payment_required: invoice.data.payment_required ?? null,
...
export function InvoiceDetail({ invoice, user, onBack, onRefresh }: { invoice: Invoice; user: User; onBack: () => void; onRefresh: (updated?: Invoice) => void }) {
  ...
  const [paymentRequired, setPaymentRequired] = useState<boolean | null>(() => typeof invoice.data.payment_required === "boolean" ? invoice.data.payment_required : null);
  ...
  const hydrate = (next: Invoice) => {
    ...
    setPaymentRequired(typeof next.data.payment_required === "boolean" ? next.data.payment_required : null);
  ...
  useEffect(() => {
    const common = Promise.all([
      ...
      api<AuditEvent[]>(`/invoices/${invoice.id}/audit`),
      api<SectionPermission[]>("/section-permissions"),
    ]);
    if (user.roles.includes("QUEUE_MANAGER")) {
      void Promise.all([
        common,
        api<UserReference[]>("/users?role=APPROVER"),
        api<PohodaConfig>("/exports/config"),
      ]).then(([[centers, history, permissions], users, config]) => {
        setCentres(centers); setApprovers(users); setSectionPermissions(permissions); setAudit(history); setPohodaConfig(config);
      });
    } else {
      void common.then(([centers, history, permissions]) => { setCentres(centers); setAudit(history); setSectionPermissions(permissions); });
    }
  }, [invoice.id, invoice.current_revision_number, user.roles]);
  useEffect(() => {
  ...
  const canPrepare = isManager || (
    user.roles.includes("APPROVER")
    && invoice.paperless.uploaded_by_subject === user.subject
    && !invoice.queue_review?.submitted_at
  );
  ...
  const call = async (key: string, path: string, init: RequestInit, success = "Uloženo.") => {
    ...
    try {
      ...
      if (typeof result === "object" && result && "validations" in result) {
        updated = result as Invoice;
        hydrate(updated);
        const validations = updated.validations;
  ...
  const saveData = () => {
    ...
    const changes: Record<string, unknown> = Object.fromEntries(Object.entries(form).map(([key, value]) => {
    ...
    changes.payment_required = paymentRequired;
  ...
  const saveClassification = () => void call(
    ...
    { method: "PUT", body: JSON.stringify({ document_type: documentType, processing_mode: processingMode, payment_required: paymentRequired, expected_revision: hydratedRevision.current }) },
    "Klasifikace, režim a údaj K zaplacení byly uloženy.",
  );
  ...
  const roundingValidation = vatValidations.find((row)=>row.code==="VAT_ROUNDING_ADJUSTMENT");
  const roundingRows = roundingValidation?.details?.rounding;
  const roundingRowIndex = Number(Array.isArray(roundingRows) ? (roundingRows[0] as {row?:number})?.row ?? 0 : 0) - 1;
  const roundingLine = roundingRowIndex >= 0 ? vatLines[roundingRowIndex] : undefined;
  return <section>
    ...
    <div className="detail-grid">
      ...
      <div className="work-panel">
        <div className="card classification-card"><div className="card-title"><div><h2>Klasifikace a zpracování</h2><p>Typ dokladu a režim jsou nezávislé údaje.</p></div><StatusBadge value={invoice.classification.extraction_source}/></div>
          <div className="form-grid"><label>Typ dokladu<select disabled={!canPrepare||Boolean(pending)} value={documentType} onChange={(e)=>{setDocumentType(e.target.value);setDirty(true)}}>{documentTypeOptions.map((value)=><option key={value} value={value}>{documentTypeLabel(value)}</option>)}</select></label><label>Režim<select disabled={!canPrepare||Boolean(pending)} value={processingMode} onChange={(e)=>{setProcessingMode(e.target.value);setDirty(true)}}>{processingModeOptions.map((value)=><option key={value} value={value}>{processingModeLabel(value)}</option>)}</select></label><fieldset className="payment-required"><legend>K zaplacení</legend><label><input type="radio" name="payment-required" checked={paymentRequired===true} disabled={!canPrepare||Boolean(pending)} onChange={()=>{setPaymentRequired(true);setDirty(true)}}/> Ano</label><label><input type="radio" name="payment-required" checked={paymentRequired===false} disabled={!canPrepare||Boolean(pending)} onChange={()=>{setPaymentRequired(false);setDirty(true)}}/> Ne</label>{paymentRequired===null&&<small>Neurčeno – před předáním je nutná volba.</small>}</fieldset></div>
          {canPrepare&&<button className="button secondary" disabled={Boolean(pending)} onClick={saveClassification}>{pending==="classification"?"Ukládám…":"Uložit klasifikaci"}</button>}
          <dl className="metadata-grid"><div><dt>Zdroj dat</dt><dd>{extractionSourceLabel(invoice.classification.extraction_source)}</dd></div><div><dt>ISDOC</dt><dd>{invoice.isdoc.status==="VALID"?"Nalezen a validní":isdocStatusLabel(invoice.isdoc.status)}{invoice.isdoc.version?` · ${invoice.isdoc.version}`:""}</dd></div><div><dt>Import do POHODY</dt><dd>{pohodaImportMethodLabel(invoice.classification.pohoda_import_method)}</dd></div><div><dt>Schválená kopie</dt><dd>{invoice.approved_pdf?.status?displayLabel(invoice.approved_pdf.status):"Zatím nevytvořena"}</dd></div></dl>
        ...
        <div className="card ai-card"><div className="card-title"><div><h2>Strukturovaná extrakce</h2><p>ISDOC má přednost; OCR + AI se používá pouze jako fallback.</p></div><StatusBadge value={invoice.classification.extraction_source}/></div>
          ...
          {invoice.ai.history.length > 1 && <details><summary>Historie AI běhů ({invoice.ai.history.length})</summary><ol>{invoice.ai.history.map((run)=><li key={run.id}>#{run.extraction_revision} · {run.model} · {displayLabel(run.status)} · {run.duration_ms != null ? `${(run.duration_ms/1000).toFixed(2)} s` : "bez času"}{run.applied ? " · použit" : ""}</li>)}</ol></details>}
        </div>
        ...
        <div className="card invoice-data-card"><div className="card-title"><h2>Fakturační údaje</h2></div>
          ...
          <div className="form-grid">{editableFields.map(([key, label]) => {const fieldMessage=fieldErrors[key]||validationByField[key]?.message;const change=(value:string)=>{setForm({...form,[key]:value});setDirty(true);setFieldErrors({...fieldErrors,[key]:""})};return <label key={key} className={`${key==="description"?"form-note ":""}${fieldMessage?"field-invalid":""}`.trim()}>{label}{key==="description"?<textarea data-field={key} rows={3} wrap="soft" aria-invalid={Boolean(fieldMessage)} disabled={!canPrepare||Boolean(pending)} value={form[key]} onChange={(event)=>change(event.target.value)} />:dateFields.has(key)?<CzechDateInput field={key} invalid={Boolean(fieldMessage)} disabled={!canPrepare||Boolean(pending)} value={form[key]} onChange={change}/>:<input data-field={key} aria-invalid={Boolean(fieldMessage)} disabled={!canPrepare||Boolean(pending)} value={form[key]} onChange={(event)=>change(event.target.value)} />}{fieldMessage&&<small className="field-error">{fieldMessage}</small>}{evidence[key] && <small title={evidence[key]}>AI zdroj: {evidence[key]}</small>}</label>})}{canPrepare&&<div className="form-actions"><button className="button secondary" disabled={Boolean(pending)} onClick={saveData}>{pending==="data"?"Ukládám…":"Uložit změny"}</button></div>}</div>
        </div>
        <div className="card vat-card"><div className="card-title"><div><h2>DPH a zaokrouhlení</h2><p>Hodnoty vytištěné na faktuře zůstávají autoritativní; přepočet je pouze kontrola.</p></div></div>
          <dl className="metadata-grid"><div><dt>Základ z faktury</dt><dd>{money(String(invoice.data.total_without_vat||""),String(invoice.data.currency||"CZK"))}</dd></div><div><dt>DPH z faktury</dt><dd>{money(String(invoice.data.total_vat||""),String(invoice.data.currency||"CZK"))}</dd></div><div><dt>Celkem z faktury</dt><dd>{money(String(invoice.data.total_amount||""),String(invoice.data.currency||"CZK"))}</dd></div><div><dt>Zaokrouhlení</dt><dd>{invoice.data.rounding_amount==null?"Neurčeno":money(String(invoice.data.rounding_amount),String(invoice.data.currency||"CZK"))}</dd></div></dl>
          {vatLines.length>0&&<div className="vat-lines">{vatLines.map((row,index)=>{const isRounding=index===roundingRowIndex;return <div key={index} className={isRounding?"vat-line rounding":"vat-line"}><strong>{isRounding?"Zaokrouhlení":`DPH řádek ${index+1}`}</strong><span>Základ {shown(row.taxable_base??row.base)} · sazba {shown(row.vat_rate??row.rate)} % · DPH {shown(row.vat_amount??row.vat)} · celkem {shown(Number(row.taxable_base??row.base??0)+Number(row.vat_amount??row.vat??0))}</span></div>})}</div>}
          {roundingValidation&&<div className="alert warning"><strong>⚠ Doklad obsahuje explicitní zaokrouhlení.</strong> Částka {shown(roundingValidation.actual)} Kč.</div>}
        ...
        <div className="card"><div className="card-title"><div><h2>Sekce dokladu</h2><p>Uploader může použít libovolnou aktivní sekci. Oprávnění určuje, kterou část při předání automaticky schválí.</p></div>{canPrepare&&<button className="button secondary" disabled={Boolean(pending)} onClick={()=>changeAllocationRows([...allocationRows,{cost_center_id:centres[0]?.id || "",amount:"0.00",percentage:"0",note:"",vat_breakdown:"[]"}])}>Přidat řádek</button>}</div>
          ...
          {canPrepare&&<div className="mode-switch"><label><input type="radio" checked={allocationMode==="amount"} onChange={()=>{setAllocationMode("amount");setDirty(true)}}/> Částkou</label><label><input type="radio" checked={allocationMode==="percentage"} onChange={()=>{setAllocationMode("percentage");setDirty(true)}}/> Procentem</label></div>}
          {canPrepare&&allocationRows.map((row,index)=>{const own=sectionPermissions.some((permission)=>permission.active&&permission.approver_subject===user.subject&&permission.cost_center.id===row.cost_center_id);return <div className="allocation-row" key={index}><select aria-label={`Sekce rozúčtování ${index+1}`} value={row.cost_center_id} onChange={(e)=>changeAllocationRows(allocationRows.map((r,i)=>i===index?{...r,cost_center_id:e.target.value}:r))}><option value="">Vyberte sekci</option>{centres.map((c)=>{const eligible=sectionPermissions.some((permission)=>permission.active&&permission.approver_subject===user.subject&&permission.cost_center.id===c.id);return <option value={c.id} key={c.id}>{c.code} — {c.name}{!isManager?eligible?" · Můžete schválit":" · Vyžaduje jiného schvalovatele":""}</option>})}</select>{!isManager&&row.cost_center_id&&<small className={own?"permission-own":"permission-other"}>{own?"Při předání automaticky schválíte":"Vyžaduje schválení jiným schvalovatelem"}</small>}<input aria-label={allocationMode==="amount"?"Částka":"Procento"} inputMode="decimal" value={allocationMode==="amount"?row.amount:row.percentage} onChange={(e)=>changeAllocationRows(allocationRows.map((r,i)=>i===index?{...r,[allocationMode]:e.target.value}:r))}/><input aria-label="Poznámka" placeholder="Poznámka" value={row.note} onChange={(e)=>changeAllocationRows(allocationRows.map((r,i)=>i===index?{...r,note:e.target.value}:r))}/><textarea aria-label="DPH rozpad allocation" title="Povinné jen při více sazbách a více sekcích" value={row.vat_breakdown} onChange={(e)=>changeAllocationRows(allocationRows.map((r,i)=>i===index?{...r,vat_breakdown:e.target.value}:r))}/><button className="icon-button" aria-label="Odebrat" onClick={()=>changeAllocationRows(allocationRows.filter((_,i)=>i!==index))}>×</button></div>})}
          {canPrepare&&<button className="button secondary" disabled={Boolean(pending)} onClick={saveAllocations}>{pending==="allocations"?"Ukládám…":"Uložit sekce"}</button>}
          {invoice.allocations.map((allocation)=><div className="assignment-summary" key={allocation.id}><strong>{allocation.cost_center.code}: {money(allocation.amount,String(invoice.data.currency||"CZK"))}{allocation.percentage!=null?` · ${allocation.percentage} %`:""}</strong>{allocation.note&&<small>{allocation.note}</small>}{isManager&&<div className="approver-picker">{approvers.filter((approver)=>sectionPermissions.some((permission)=>permission.active&&permission.approver_subject===approver.subject&&permission.cost_center.id===allocation.cost_center.id)).map((approver)=><label className="check-row" key={approver.subject}><input type="checkbox" disabled={Boolean(pending)} checked={(approverChoices[allocation.id]||[]).includes(approver.subject)} onChange={(e)=>{setApproverChoices({...approverChoices,[allocation.id]:e.target.checked?[...(approverChoices[allocation.id]||[]),approver.subject]:(approverChoices[allocation.id]||[]).filter(x=>x!==approver.subject)});setDirty(true)}}/><span>{approver.username}</span></label>)}<button className="button secondary" disabled={Boolean(pending)} onClick={()=>saveApprovers(allocation.id)}>{pending===`approvers-${allocation.id}`?"Ukládám…":"Uložit schvalovatele"}</button></div>}<span>{allocation.assignments.length ? allocation.assignments.map((a)=>`${approvers.find(x=>x.subject===a.approver_subject)?.username||a.approver_subject} (${displayLabel(a.status)})`).join(", ") : "Schvalovatel není přiřazen"}</span></div>)}
        </div>
        {isManager&&<div className="card actions"><h2>Kontrola originálu a workflow</h2>{invoice.paperless.upload_origin==="APPROVER"&&<div className="alert info"><strong>Nahrál schvalovatel:</strong> {invoice.paperless.uploaded_by}. {invoice.queue_review?.submitted_at?`Předáno ${pragueDateTime(invoice.queue_review.submitted_at)}.`:"Zatím nepředáno ke kontrole."}</div>}<p>{invoice.original_review_confirmed ? `Originál zkontroloval ${invoice.original_reviewed_by} (${pragueDateTime(invoice.original_reviewed_at)}).` : "Originál zatím nebyl explicitně potvrzen jako zkontrolovaný."}</p><div><button className="button secondary" disabled={!actionable||Boolean(pending)} onClick={()=>void call("original", `/invoices/${invoice.id}/confirm-original`,{method:"POST"},"Kontrola originálu byla potvrzena.")}>{pending==="original"?"Potvrzuji…":"Potvrdit kontrolu originálu"}</button><button className="button primary" disabled={!actionable||Boolean(pending)} onClick={()=>void call("submit", `/invoices/${invoice.id}/submit`,{method:"POST"},"Revize byla zkontrolována a předána ke schválení.")}>{pending==="submit"?"Předávám…":"Schválit kontrolu a předat"}</button>{invoice.status==="REJECTED"&&<button className="button warning" disabled={!actionable||Boolean(pending)} onClick={()=>void call("reopen", `/invoices/${invoice.id}/reopen`,{method:"POST"},"Faktura byla znovu otevřena.")}>{pending==="reopen"?"Otevírám…":"Znovu otevřít zamítnutou fakturu"}</button>}</div></div>}
        {!isManager&&invoice.paperless.uploaded_by_subject===user.subject&&<div className="card actions"><h2>Příprava a předání</h2>{invoice.queue_review?.submitted_at?<div className="alert info"><strong>Čeká na kontrolu správce fronty.</strong> Předáno {pragueDateTime(invoice.queue_review.submitted_at)}. Případné změny vytvoří správce fronty v nové revizi.</div>:<><p>Zkontrolujte originál, zvolte typ a K zaplacení a rozdělte náklad. Při předání se vaše oprávněné části automaticky schválí; ostatní dostanou schvalovatele po kontrole správce fronty.</p><div><button className="button secondary" disabled={!actionable||Boolean(pending)} onClick={()=>void call("original", `/invoices/${invoice.id}/confirm-original`,{method:"POST"},"Kontrola originálu byla potvrzena.")}>{pending==="original"?"Potvrzuji…":"Potvrdit kontrolu originálu"}</button><button className="button primary" disabled={!actionable||Boolean(pending)} onClick={()=>void call("queue-review", `/invoices/${invoice.id}/submit-for-review`,{method:"POST"},"Doklad byl předán správci fronty a vaše oprávněné části byly schváleny.")}>{pending==="queue-review"?"Předávám…":"Předat správci fronty"}</button></div></>}</div>}
