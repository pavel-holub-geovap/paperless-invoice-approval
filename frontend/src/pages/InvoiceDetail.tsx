    ...
    <div className="detail-grid">
      ...
      <div className="work-panel">
        <div className="card classification-card"><div className="card-title"><div><h2>Klasifikace a zpracování</h2><p>Typ dokladu a režim jsou nezávislé údaje.</p></div><StatusBadge value={invoice.classification.extraction_source}/></div>
          <div className="form-grid"><label>Typ dokladu<select disabled={!canPrepare||Boolean(pending)} value={documentType} onChange={(e)=>setDocumentType(e.target.value)}>{documentTypes.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Režim<select disabled={!canPrepare||Boolean(pending)} value={processingMode} onChange={(e)=>setProcessingMode(e.target.value)}>{processingModes.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></div>
        ...
        <div className="card"><div className="card-title"><h2>Fakturační údaje</h2>{canPrepare&&<button className="button secondary" disabled={Boolean(pending)} onClick={saveData}>{pending==="data"?"Ukládám…":"Uložit změny"}</button>}</div>
