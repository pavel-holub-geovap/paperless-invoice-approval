...
export function CostCenters() {
  ...
  return <section>
    <div className="section-heading"><div><p className="eyebrow">Administrátorský číselník a oprávnění</p><h2>Sekce a oprávnění schvalovatelů</h2><p className="muted">Globální sekce a vazby schvalovatelů spravuje ADMIN. Správce fronty je pouze používá u konkrétního dokladu.</p></div></div>
    ...
    <div className="table-wrap"><table><thead><tr><th>Kód</th><th>Název</th><th>POHODA</th><th>Stav</th><th>Akce</th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td><input aria-label={`Kód sekce ${row.code}`} value={row.code} onChange={(event)=>setRows(current=>current.map(item=>item.id===row.id?{...item,code:event.target.value}:item))}/></td><td><input aria-label={`Název sekce ${row.code}`} value={row.name} onChange={(event)=>setRows(current=>current.map(item=>item.id===row.id?{...item,name:event.target.value}:item))}/></td><td><input aria-label={`POHODA kód sekce ${row.code}`} value={row.pohoda_code} onChange={(event)=>setRows(current=>current.map(item=>item.id===row.id?{...item,pohoda_code:event.target.value}:item))}/></td><td>{row.active?"Aktivní":"Neaktivní"}</td><td><div className="admin-row-actions"><button className="button primary" onClick={()=>void update(row)}>Uložit</button><button className="button secondary" onClick={()=>void update(row,{active:!row.active})}>{row.active?"Deaktivovat":"Aktivovat"}</button></div></td></tr>)}</tbody></table></div>
