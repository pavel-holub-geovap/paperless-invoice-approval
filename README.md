...
Opravná iterace po Etapě F normalizuje český účet deterministicky na `bank_account_raw`, `bank_account_prefix`, `bank_account_number`, `bank_code` a zpětně kompatibilní `bank_account`. Schéma v2 odděluje dodavatelský `supplier_address_raw` na street/city/zip pouze z dodavatelského bloku a eviduje samostatný `ROUNDING` VAT řádek. Matematické reconciliation rozdíly jsou review WARNING; pouze neplatný nebo neúplný formát VAT řádku zůstává blokující. Správce má zvláštní pohledy aktivních, ignorovaných a zdrojově chybějících dokladů. Frontend používá skutečné SPA URL včetně přímého `/invoices/{id}`.
