...
def approve_current(
    ...
    for name in ("approver1", "approver2", "approver3"):
        rows = response_json(clients[name].get(f"{base_url}/api/approvals/mine"), f"{name} tasks")
...
def xml_semantics(xml: bytes, expected: dict[str, Any]) -> dict[str, Any]:
    ...
    require(root.find(".//inv:number", NS) is None, "Supplier invoice generated a POHODA internal number")
    address = root.find(".//inv:partnerIdentity/typ:address", NS)
    require(address is not None and address.get("linkToAddress") == "false", "Free address is not explicit")
    address_values = {
        key: address.findtext(f"typ:{key}", namespaces=NS)
        for key in ("company", "street", "city", "zip", "ico", "dic")
    }
    require(address_values["street"] == expected["supplier_street"], "Supplier street changed")
    require(address_values["city"] == expected["supplier_city"], "Supplier city changed")
    require(address_values["zip"] == expected["supplier_zip"], "Supplier ZIP changed")
    items: list[dict[str, str]] = []
    for item in root.findall(".//inv:invoiceItem", NS):
        base = Decimal(item.findtext("inv:homeCurrency/typ:price", namespaces=NS) or "0")
        vat = Decimal(item.findtext("inv:homeCurrency/typ:priceVAT", namespaces=NS) or "0")
        items.append(
            {
    ...
    require({row["centre"] for row in items} == {"200", "300"}, "Allocation centres are wrong")
    require(sum((Decimal(row["gross"]) for row in items), Decimal(0)) == Decimal("1210.00"), "Item total is wrong")
...
def main() -> None:
    ...
    try:
        ...
        changes = {
            key: value for key, value in structured_address.items() if current["data"].get(key) != value
        }
        if changes:
            current = response_json(
                api(
                    ...
                    manager_user,
                    payload={"changes": changes, "comment": "Přesná adresa ověřena v originálním PDF"},
                ),
        ...
        if current["status"] == "AWAITING_APPROVAL":
            ...
            current = detail(manager, base_url, invoice_id)
        require(current["status"] in {"APPROVED", "READY_FOR_EXPORT", "EXPORT_CREATED"}, "Invoice is not exportable")
        ...
        require(
            {row["cost_center"]["pohoda_code"] for row in current["allocations"]} == {"200", "300"},
        ...
        require(first["status"] == "XSD_VALID", f"First XML is invalid: {first['validation_errors']}")
        first_xml = manager.get(f"{base_url}/api/exports/artifacts/{first['id']}/xml")
        require(first_xml.status_code == 200, "First immutable XML cannot be downloaded")
        require(hashlib.sha256(first_xml.content).hexdigest() == first["xml_sha256"], "First XML hash mismatch")
        ...
        require(second["source_export_id"] == first["id"], "Re-export did not link its source artifact")
        require(second["xml_sha256"] == first["xml_sha256"], "Deterministic re-export changed XML bytes")
        xml_response = manager.get(f"{base_url}/api/exports/artifacts/{second['id']}/xml")
        require(xml_response.status_code == 200, "Re-export XML cannot be downloaded")
        semantics = xml_semantics(xml_response.content, {**current["data"], **structured_address})

        pdf = manager.get(f"{base_url}/api/invoices/{invoice_id}/pdf")
        require(pdf.status_code == 200 and pdf.content.startswith(b"%PDF"), "Original Paperless PDF is unavailable")
        require(hashlib.sha256(pdf.content).hexdigest() == second["pdf_sha256"], "PDF snapshot hash mismatch")
        ...
        require(batch["items"][0]["export_artifact_id"] == second["id"], "Batch did not use latest XML")
        ...
        require(hashlib.sha256(archive.content).hexdigest() == batch["archive_sha256"], "ZIP hash mismatch")
        with zipfile.ZipFile(io.BytesIO(archive.content)) as package:
            names = sorted(package.namelist())
            require(names == sorted([batch["items"][0]["pdf_filename"], batch["items"][0]["xml_filename"]]), "ZIP layout is unstable")
            require(package.read(batch["items"][0]["xml_filename"]) == xml_response.content, "ZIP XML differs")
            require(package.read(batch["items"][0]["pdf_filename"]) == pdf.content, "ZIP PDF differs")

        response_path = Path(os.environ.get("POHODA_RESPONSE_SAMPLE", "/app/pohoda-xsd/samples/received-invoice-response.xml"))
        require(response_path.is_file(), f"POHODA response fixture is missing: {response_path}")
        response_upload = manager.post(
            ...
            files={"response_file": ("received-invoice-response.xml", response_path.read_bytes(), "application/xml")},
        )
        require(response_upload.status_code == 201, f"Response upload failed: {response_upload.text[:500]}")
        parsed = response_upload.json()
        require(parsed["parse_status"] == "PARSED", f"Response parser failed: {parsed['parse_errors']}")
        require(detail(manager, base_url, invoice_id)["status"] == "EXPORT_CREATED", "Response upload changed workflow state")

        audit_rows = response_json(manager.get(f"{base_url}/api/invoices/{invoice_id}/audit"), "audit")
