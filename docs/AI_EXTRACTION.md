...
Kanonické striktní schéma je `invoice-extraction.v3`. Ollama však nejprve produkuje stejně přesně známou strukturu `InvoiceExtractionRawV1`; její skalární hodnoty smějí zachovat nezpracovaný JSON/string formát modelu. Samostatná deterministická vrstva teprve poté vytvoří `InvoiceExtractionV1`. Kanonické schéma obsahuje dodavatele, samostatné `supplier_address_raw`, `supplier_street`, `supplier_city`, `supplier_zip`, číslo faktury, variabilní symbol, data, platební údaje, VAT řádky, deklarované součty a explicitní `rounding_amount`. VAT řádek rozlišuje `taxable_base`, `vat_amount`, `gross_amount`; historický `adjustment_type=ROUNDING` už není autoritou current validace. `rounding_amount` se z AI přijme pouze tehdy, když jeho `source_text` obsahuje samostatný explicitní štítek a stejnou částku. Nula je platná hodnota bez warningu; CELKEM ani MEZISOUČET nejsou evidence. Ruční hodnota v aktuální revizi je autoritativní a AI se kvůli její opravě znovu nespouští. Pokud model prokazatelně zamění hrubou částku za DPH, normalizace zachová `vat_amount_extracted` a transparentně použije `gross_amount − taxable_base`; deklarované součty faktury se tím nemění. Každá skalární hodnota má `value` a krátký doslovný `source_text`; neznámá hodnota je explicitně `null`. Historické výsledky se zachovávají a při použití procházejí kompatibilním převodem.
