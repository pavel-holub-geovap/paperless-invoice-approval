...
Kanonické striktní schéma je `invoice-extraction.v3`. Ollama však nejprve produkuje stejně přesně známou strukturu `InvoiceExtractionRawV1`; její skalární hodnoty smějí zachovat nezpracovaný JSON/string formát modelu. Samostatná deterministická vrstva teprve poté vytvoří `InvoiceExtractionV1`. Kanonické schéma obsahuje dodavatele, samostatné `supplier_address_raw`, `supplier_street`, `supplier_city`, `supplier_zip`, číslo faktury, variabilní symbol, data, platební údaje, VAT řádky a deklarované součty. VAT řádek rozlišuje `taxable_base`, `vat_amount`, `gross_amount` a může mít `adjustment_type=ROUNDING`. Model je pouze navrhovatel: typ `ROUNDING` se kanonicky přijme jen tehdy, když `source_text` obsahuje samostatný explicitní štítek `Zaokrouhlení`, `Zaokr.`, `Haléřové vyrovnání`, `Vyrovnání` nebo `Rounding`, nenulovou částku na témže řádku a tato částka odpovídá kandidátu. `Zaokrouhlení 0,00` je bez user-facing warningu a souhrnné štítky CELKEM, CELKEM K ÚHRADĚ, Celkem s DPH, Základ, Výše DPH a Cena celkem nejsou rounding evidence. Odmítnutý typ zůstává v raw odpovědi a v `normalization_result.rejections` s evidence a důvodem, ale vlastní VAT řádek zůstane běžným účetním řádkem. Při každém ručním uložení se odvozený příznak znovu ověří, takže historický AI kandidát nemůže přejít do nové current revize bez platné evidence. Pokud model prokazatelně zamění hrubou částku za DPH, normalizace zachová `vat_amount_extracted` a transparentně použije `gross_amount − taxable_base`; deklarované součty faktury se tím nemění. Každá skalární hodnota má `value` a krátký doslovný `source_text`; neznámá hodnota je explicitně `null`. Extra nebo chybějící pole se odmítají. Historické v1/v2 výsledky zůstávají beze změny a při případném použití procházejí kompatibilním převodem.
