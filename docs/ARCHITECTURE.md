...
Ruční save fakturačních dat nepoužívá validační snapshot AI běhu. V jedné transakci uloží data do nové nebo současné `InvoiceRevision`, provede flush a spustí celý deterministický validační set explicitně nad current revizí. API proto nikdy nekombinuje nová business data s validacemi předchozí revize a ruční oprava nespouští nový AI běh.
